...
public class Game06_Main : MonoBehaviour
{
    public Game06_GameUIComm gameUIComm;
    public Game06_GameUI gameUI_Single;
    public Game06_GameUI gameUI_MulitPlayer;
    Game06_GameUI gameUI;
    public GameObject shakeMain_Obj;
    public Game06_Player[] player;
    ...
    public static Game06_Main instance;
        ...
        switch (statue)
        {
            ...
            case en_Game00_Sta.WaitStart:
                ...
                if (runTime > 5)
                {
                    ...
                    if (!Game_Map06.instance.audioSource_Others.isPlaying)
                    {
            ...
            case en_Game00_Sta.Play:
                CheckLedKey();
                if (Game_Map06.instance.zadashu <= 0 && Main.MapIndex == 6)
                {
                    runTime += Time.deltaTime;
                    if (runTime > 1)
                    {
             ...
             //   if (!Main.IsDemo && Set.setVal.TimeMode == 0) { Main.PlayTime -= Time.deltaTime; }
                ...
                //
                if (Index_JieDuan >= Game_Map06.instance.MaxJieDuan)// Game_Map06.instance.MaxJieDuan
                {
                    ChangeStatue(en_Game00_Sta.ShowResult);
                ...
                if (Game_Map06.instance.isClearAll)
                {
                    if (Main.IsDemo)
                    {
                        ChangeStatue(en_Game00_Sta.Out);
            ...
            case en_Game00_Sta.ShowResult:
                ...
                if (runTime >= 2)
                {
                    ...
                    {
                        if (LedKey.KeyPressed(Game06_Main.instance.targetKeyId) || Input.GetKeyDown(KeyCode.Q))
                        {
    ...
    public void ChangeStatue(en_Game00_Sta sta)
    {
        ...
        switch (statue)
        {
            ...
            case en_Game00_Sta.Play:
                ...
                Game_Map06.instance.InitMap(Main.MapIndex);
                break;
            ...
            case en_Game00_Sta.ShowResult:
                ...
                if (Game_Map06.instance.remainPoint > 0 && Main.MapIndex == 6)
                {
                    FjData.g_Fj[0].Life = 0;
                }
                if (FjData.g_Fj[0].Life > 0)
    ...
    public bool HitTargetPoint(int no, int x, int y)
    {
        ...
        if (FjData.g_Fj[no].RemainPoint > 0 && FjData.g_Fj[no].Life > 0)
        {
            if (Main.MapIndex == 6)
            {
                ...
                if (Game_Map06.instance.zadashu > 0)
                {
    ...
    void CheckLedKey()
    {
        ...
        for (x = 0; x < Set.setVal.Width; x++)
        {
            ...
            if (Framebuffer.led[pointId].statue == enPointSta.Target)
            {
                    ...
                    if (Main.MapIndex == 1)
                    {
                        if (Game_Map06.instance.OldPoint < 0)
                        {

                            Game_Map06.instance.OldPoint = pointId;
                            pressTargetPoint = false;

                        }
                        else
                        {
                            if (Game_Map06.instance.OldPoint != pointId)
                            {
                                if (Game_Map06.instance.Led_Color[Game_Map06.instance.OldPoint] == Game_Map06.instance.Led_Color[pointId])
                                {
                                    GameLedControl.gamePoint[pointId].tarageTime = 0.2f;
                                    ...
                                    Game_Map06.instance.Led_Color[pointId] = 0;
                                    Game_Map06.instance.Led_Color[Game_Map06.instance.OldPoint] = 0;

                                    GameLedControl.gamePoint[pointId].statue = enPointSta.None;
                                    GameLedControl.gamePoint[Game_Map06.instance.OldPoint].statue = enPointSta.None;

                                    Framebuffer.Update_PointColor(pointId, 0, enPointSta.None);
                                    Framebuffer.Update_PointColor(Game_Map06.instance.OldPoint, 0, enPointSta.None);


                                    Game_Map06.instance.OldPoint = -1;
                                    pressTargetPoint = false;
                    ...
                    else if (Main.MapIndex == 2)
                    {
                        if (Game_Map06.instance.ShackCnt < -2)
                        {
                            Game_Map06.instance.isClearAll = true;
                    ...
                    else
                    {
                        if (Main.MapIndex == 3)
                        {
                            if (Game_Map06.instance.ShackCnt > 0)
                            {
                                return;
                        ...
                        for (int j = 0; j < 1; j++)
                        {
                            if (HitTargetPoint(j, x, y))
                            {
                                ...
                                if (Main.MapIndex == 6)
                                {
                                    if (Game_Map06.instance.zadashu > 0)
                                    {
                                        Game_Map06.instance.zadashu--;
                                        //Game_Map06.instance.Update_Zadan();
                                        Game_Map06.instance.zadanx[Game_Map06.instance.zadanzuobiao] = x;
                                        Game_Map06.instance.zadany[Game_Map06.instance.zadanzuobiao] = y;
                                        Game_Map06.instance.zadanzuobiao++;
                                        if (Game_Map06.instance.zadashu == 0) //Game_Map06.instance.zadashu <= 0
                                        {
                                            runTime = 0;
                                            Game_Map06.instance.startBooming = true;
                                            return;
                                        }
                                    }
                                ...
                                if (Main.MapIndex == 9)
                                {
                                    if (Game_Map06.instance.IsOver_new09)
                                    {
                                        return;
                                    }
                                    if (Game_Map06.instance.isPass)
                                    {
                                        //Debug.LogError(pointId+"   准备过关!!!!" + Game_Map06.instance.EndPos);
                                        if (pointId == Game_Map06.instance.EndPos)
                                        {
                                            // Debug.LogError("过关!!!!");
                                            Game_Map06.instance.IsOver_new09 = true;
                                            return;
                                        }

                                    }
                                    if (Game_Map06.instance.Led_Color[pointId] == 0x00ff00) 
                                    {
                                    ...
                                    if (pointId == Game_Map06.instance.EndPos)
                                    {
                                        Game_Map06.instance.isPass = true;
    ...
    public void NextJieDuan()
    {
        ...
        remainTime = maxremainTime = SettingInGame_02.instance.set_LevelTime;
        Game_Map06.instance.isClearAll = false;//是否清理了所有的点
