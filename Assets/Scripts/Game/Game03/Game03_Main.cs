...
public class Game03_Main : MonoBehaviour
{
    public Game03_GameUIComm gameUIComm;
    public Game03_GameUI gameUI_Single;
    public Game03_GameUI gameUI_MulitPlayer;
    Game03_GameUI gameUI;
    public GameObject shakeMain_Obj;
    public Game03_Player[] player;
    ...
    public static Game03_Main instance;
    ...
    void Update()
    {
        ...
        switch (statue)
        {
            ...
            case en_Game00_Sta.Play:
                ...
                {
                //   Main.PlayTime -= Time.deltaTime;
                    //if (Main.PlayTime <= 0) Main.PlayTime = -1;
                }
                ...
                //
                if (remainTime > 0 && Game_Map03.instance.isMoving == false)
                {
                ...
                //
                if (Index_JieDuan >= Game_Map03.instance.MaxJieDuan)// 
                {
                    ChangeStatue(en_Game00_Sta.ShowResult);
                ...
                if (Game_Map03.instance.isClearAll)
                {
                    if (Main.IsDemo)
                    {
                        ChangeStatue(en_Game00_Sta.Out);
                    ...
                    else
                    {
                        ...
                        Game_Map03.instance.isCleaning = true;


                        if (Game_Map03.instance.Run_MapClear())
                        {
                ...
                {
                    if (Game_Map03.instance.remainPoint <= 0 && !Game_Map03.instance.isinfinite)
                    {
                ...
                if (remainTime <= 0)
                {
                    if (!Game_Map03.instance.isinfinite)
                    {
                        FjData.g_Fj[0].Life = 0;
                        ChangeStatue(en_Game00_Sta.WaitContinue);
                        break;
                    }
                    else if (Game_Map03.instance.isinfinite && Game_Map03.instance.remainPoint <= 0)
                    {
                        ChangeStatue(en_Game00_Sta.ShowResult);
                        Game_Map03.instance.isinfinite = false;
                        break;
                    }
                    else if (Game_Map03.instance.isinfinite && Game_Map03.instance.remainPoint > 0)
                    {
            ...
            case en_Game00_Sta.ShowResult:
                ...
                if (runTime >= 2)
                {
                    ...
                    {
                        // if (LedKey.KeyPressed(Game03_Main.instance.targetKeyId) || Input.GetKeyDown(KeyCode.Q))
                        {
                            Main.MapIndex++;
                            if (Main.MapIndex >= 29)
                            {
    ...
    public void ChangeStatue(en_Game00_Sta sta)
    {
        ...
        switch (statue)
        {
            ...
            case en_Game00_Sta.Play:
                ...
                Game_Map03.instance.InitMap(gameLevel);
                break;
    ...
    void CheckLedKey()
    {
...
#endif
        if (Game_Map03.instance.isMoving)
        {
            return;
        }
        for (x = 0; x < Set.setVal.Width; x++)
        {
            for (y = 0; y < Set.setVal.Height; y++)
            {
                ...
                //list_PresetPic[id].color = updateColor(Framebuffer.led[id].color);
                ...
                if (Game_Map03.instance.tobeachtime[pointId] > 0)
                {
                    continue;
                        ...
                        if (HitDiePoint(j, x, y))
                        {
                            ...
                            if (FjData.g_Fj[0].Life > 0)
                            {
                                if (errorCD <= 0)
                                {
                                    ...
                                    Game_Map03.instance.tobeachtime[pointId] = 0.3f;
                                    errorCD = 0.2f;
            ...
            if (Framebuffer.led[pointId].statue == enPointSta.Target)
            {
                    ...
                    for (int j = 0; j < 1; j++)
                    {
                        ...
                        {
                            int trueID = pointId / 3;
                            Game_Map03.instance.MoveIndex++;

                            Game_Map03.instance.have_touch[trueID] = true;
                            Game_Map03.instance.tobeachtime[pointId] = 0.3f;


                            if (Game_Map03.instance.isinfinite)
                            {
                                GameLedControl.gamePoint[trueID * 3].statue = enPointSta.Die;
                                GameLedControl.gamePoint[trueID * 3 + 1].statue = enPointSta.Die;
                                GameLedControl.gamePoint[trueID * 3 + 2].statue = enPointSta.Die;
                                Framebuffer.Update_PointColor(trueID * 3, 0, enPointSta.Die);
                                Framebuffer.Update_PointColor(trueID * 3 + 1, 0, enPointSta.Die);
                                Framebuffer.Update_PointColor(trueID * 3 + 2, 0, enPointSta.Die);
                                DrawPic.DrawPointId(trueID * 3, 0, enPointSta.Die);
                                DrawPic.DrawPointId(trueID * 3 + 1, 0, enPointSta.Die);
                                DrawPic.DrawPointId(trueID * 3 + 2, 0, enPointSta.Die);


                            }
                            else
                            {
                                GameLedControl.gamePoint[trueID * 3].statue = enPointSta.None;
                                GameLedControl.gamePoint[trueID * 3 + 1].statue = enPointSta.None;
                                GameLedControl.gamePoint[trueID * 3 + 2].statue = enPointSta.None;
                                Framebuffer.Update_PointColor(trueID * 3, 0, enPointSta.None);
                                Framebuffer.Update_PointColor(trueID * 3 + 1, 0, enPointSta.None);
                                Framebuffer.Update_PointColor(trueID * 3 + 2, 0, enPointSta.None);
                                DrawPic.DrawPointId(trueID * 3, 0, enPointSta.None);
                                DrawPic.DrawPointId(trueID * 3 + 1, 0, enPointSta.None);
                                DrawPic.DrawPointId(trueID * 3 + 2, 0, enPointSta.None);

                            }

                            Game_Map03.instance.beTouch[trueID] = true;
                            if (Game_Map03.instance.remainPoint > 0)
                            {

                                Game_Map03.instance.remainPoint--;
                            }
    ...
    public void NextJieDuan()
    {
        ...
        remainTime = maxremainTime = SettingInGame_02.instance.set_LevelTime;
        Game_Map03.instance.isClearAll = false;//是否清理了所有的点
