...
public class Game07_PlayerUI : MonoBehaviour
{
    ...
    public void Awake0(int no)
    {
        ...
        //    for (int i = 0; i < len; i++) {
        //        lifeOne[i] = Instantiate (Game06_Main.instance.lifeOne_Prefab, life_Layer).GetComponent<Game00_LifeOne> ();
    ...
    void CheckLanguage () {
        if (language != Set.setVal.Language || sprite_Result == null) {
            ...
            if (Set.setVal.Language==2)
            {
                sprite_Result = Resources.LoadAll<Sprite>("UI/Result_French" );
    ...
    void Update()
    {
        ...
        if (remainPoint != FjData.g_Fj[playerId].RemainPoint) {
            Update_RemainPoint ();
        }
    ...
    public void Update_RemainPoint () {
        ...
        return;
        if (maxPoint < FjData.g_Fj[playerId].RemainPoint) {
            maxPoint = FjData.g_Fj[playerId].RemainPoint;
        }
        remainPoint = FjData.g_Fj[playerId].RemainPoint;
        text_RemainPoint.text = remainPoint.ToString ();
        if (maxPoint > 0) {
            image_RemainPointValue.fillAmount = (float)remainPoint / maxPoint;
        }
    }
